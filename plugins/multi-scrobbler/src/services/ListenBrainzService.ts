...
export class ListenBrainzService extends BaseService {
    ...
    async validateCredentials(): Promise<boolean> {
        try {
            const username = currentSettings.listenbrainzUsername || currentSettings.username;
    ...
    async fetchLatestScrobble(): Promise<Track> {
        try {
            ...
            try {
                ...
                const playingNowData: ListenBrainzPlayingNowResponse = await this.makeRequest(playingNowUrl, { headers });
            ...
            if (currentlyPlaying) {
                ...
                const data: ListenBrainzResponse = await this.makeRequest(url, { headers });
            ...
            const trackTimestamp = latestListen.listened_at || Math.floor(Date.now() / 1000);
            ...
            if (latestListen.track_metadata.additional_info?.duration_ms) {
                duration = Math.floor(latestListen.track_metadata.additional_info.duration_ms / 1000);
            ...
            const track: Track = {
                ...
                albumArt,
                url: latestListen.track_metadata.additional_info?.origin_url ||
             `https://listenbrainz.org/user/${username}`,
                date: isNowPlaying ? "now" : new Date(trackTimestamp * 1000).toISOString(),
