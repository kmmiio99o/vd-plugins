...
class LastFMClient {
  ...
  private async handleError(error: LastFMError): Promise<never> {
    ...
    console.log(`[Last.fm] API Error ${error.error}: ${message}`);
  ...
  > {
    try {
      ...
      if (!lastTrack) {
        console.log("[Last.fm] No tracks found for user");
      ...
      try {
        ...
        console.warn("[Last.fm] Failed to fetch track duration", err);
      }
      ...
      if (this.retryCount > Constants.MAX_RETRY_ATTEMPTS) {
        this.retryCount = 0;
        console.error("[Last.fm] Max retry attempts reached");
        throw error;
      }
